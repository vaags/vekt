...
TEST_CASE("Rav processor renders every mode across tracking qualities", "[processor][rav]")
{
	for (auto qualityIndex : { 0.0f, 1.0f, 2.0f })
	{
		...
		{
			vekt::saturator::PluginProcessor processor;
			setParameter(processor, vekt::saturator::parameters::trackingOversampling, qualityIndex);
			setParameter(processor, vekt::saturator::parameters::mode, modeIndex);
			setParameter(processor, vekt::saturator::parameters::drive, 30.0f);
			setParameter(processor, vekt::saturator::parameters::bias, 0.5f);
			setParameter(processor, vekt::saturator::parameters::autoGain, 1.0f);
